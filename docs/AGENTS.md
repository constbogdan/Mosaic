...
-   Local validation invokes pre-commit through the Python interpreter it already resolved: `python -m pre_commit`. It does not depend on a generated `pre-commit` launcher from `PATH`. If the module is unavailable, install it once with `python -m pip install pre-commit`; validation never mutates permanent developer tooling, security policy, or `PATH`.
