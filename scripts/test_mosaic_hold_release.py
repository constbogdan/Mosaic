...
class HoldReleaseTests(unittest.TestCase):
    ...
    def test_workflow_is_zero_input_and_has_narrow_write_boundary(self):
        ...
        self.assertIn('## Stable %s ready to hold', prepare)
        self.assertIn('stop advertising this release through <code>/releases/latest</code>', prepare)
        self.assertIn('tag, APK, manifest, and provenance will remain preserved', prepare)
        self.assertIn('Waiting for <code>release-hold</code> approval.', prepare)
        self.assertIn('## Stable held: [%s](%s)', mutation)
        self.assertIn('tag, APK, manifest, and provenance remain preserved', mutation)
        self.assertIn('Current Stable: [%s](%s)', mutation)
        self.assertIn('Current Stable: none available.', mutation)
        self.assertIn('Hold Release preparation failed', prepare)
        self.assertIn('Hold Release refused or failed', mutation)
